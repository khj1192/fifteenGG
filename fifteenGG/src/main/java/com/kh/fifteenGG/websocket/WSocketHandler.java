...
public class WSocketHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        Member login=(Member)session.getAttributes().get("member");
        
        //채팅방에 입장하면 상대방에게 입장 메세지 출력
        for(WebSocketSession one : sessionList) {
            if(one==session) continue;
            one.sendMessage(new TextMessage(login.getUserNo()+"님이 입장하셨습니다."));
        }
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        for(WebSocketSession one : sessionList) {
        	one.sendMessage(new TextMessage(session.getId()+"|"+message.getPayload()+"|"+session.getRemoteAddress()+"|"+login.getUserNo()+"|"+login.getNickName()));
        }
        //super.handleTextMessage(session, message);
    }
    
    // 사용자가 연결을 종료 했을 때 동작하는 메소드
    @Override
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        for (WebSocketSession one : sessionList) {
            if(one==session) continue;
            one.sendMessage(new TextMessage(login.getUserNo()+"님이 퇴장하셨습니다."));
        }
